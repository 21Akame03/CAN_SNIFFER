...
static uint8_t
    s_frame_queue_storage[ORACLE_QUEUE_LENGTH * sizeof(oracle_can_frame_t)];

static uint32_t s_dropped_frames;

/*
 * Purpose: Install the USB JTAG driver if it is not already installed
 */
...
/*
 * Purpose: Write the data to the USB JTAG port
 * Input: data: the data to be written, length: the length of the data
 * Output: void
 */
...
void Oracle_Setup(void) {
  ...
  s_frame_queue =
      xQueueCreateStatic(ORACLE_QUEUE_LENGTH, sizeof(oracle_can_frame_t),
                         s_frame_queue_storage, &s_frame_queue_struct);
...
/*
 * Purpose: Places the msg in a Queue, to be read by Oracle_to_laptop
 * Input: msg: the msg to be queued and Timestamp_us: the time of the msg
...
bool Oracle_QueueFrame(const twai_message_t *msg, uint64_t timestamp_us) {
  ...
  if (status != pdTRUE) {
    ...
    if (now_ms - last_warn_ms >= 1000) {
      ESP_LOGW(ORACLE_LOG_TAG, "Dropping CAN frames: total=%" PRIu32,
               s_dropped_frames);
      last_warn_ms = now_ms;
    }
...
/*
 * Rtos Task
 * Purpose: Reads the queue and outputs the messages to the USB JTAG
 * Input: void
 * Output: void
 */
void Oracle_to_laptop(void *args) {
  ...
  for (;;) {
    ...
    size_t len =
        Oracle_FormatCANFrame(&frame, json_buffer, sizeof(json_buffer));
    if (len == 0) {
      continue;
    }
